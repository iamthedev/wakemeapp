...
$(document).on('pagebeforeshow', '#map-screen', function() {
    if (!isGPSenabled()) {
        alert('Whoops! Please enable your GPS so we can track you better.');
        navigator.app.exitApp();
    }
    getLoadingIcon('show', 'Where you ah?');
    watching = getCurrentLocation();
});

$('#map_canvas').gmap().bind('init', function(evt, map) {
    
    if (!localStorage.getItem("currentLat") && (!localStorage.getItem("currentLong"))) {
        getLoadingIcon('show', 'Cannot find you eh..');
        watching = getCurrentLocation();
    }
    ...
    var lon = localStorage.getItem("currentLong");

    var yourCoordinates = new google.maps.LatLng(lat, lon);
    $('#map_canvas').gmap('addMarker', {'position': yourCoordinates, 'bounds': true});
});

function getLoadingIcon(visibility, textmessage) {
...
function getPosition(position) {
    ...
    localStorage.setItem("currentAlt", position.coords.altitude)
}
